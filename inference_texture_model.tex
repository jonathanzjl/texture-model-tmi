...
\author{Zhijin~Li, Ann-Katherine Carton, Serge Muller
  and~Agnès~Desolneux,% <-this % stops a space
...
% input segmented bCT VOI is shown in Figure XXX.
...
\begin{figure}[!htb]
  ...
  \subfloat[{VOI \#3}]{\includegraphics[width=0.165\textwidth]%
    {figure/all/dataset_3/pcf_new}}%
  \subfloat[{VOI \#7}]{\includegraphics[width=0.165\textwidth]%
    {figure/all/dataset_7/pcf_new}}%
  \subfloat[{VOI \#11}] {\includegraphics[width=0.165\textwidth]%
...
As discussed in Section~\ref{sec:infer-step:-fitt-1}, a 3D Mat\'ern
cluster process was fitted to each set of reconstructed ellipsoid
centers using the MCE method. Figure~\ref{fig:matern-pcf-fit} compares
the empirical PCFs estimated from the reconstructed ellipsoid centers
for the four bCT VOIs shown in Figure~\ref{fig:bct-datasets-recon} and
the theoretical PCFs of the Mat\'ern cluster processes fitted to the
same VOIs. The PCF envelopes of the fitted Mat\'ern cluster processes
were also generated using the method described in
Section~\ref{chap5-sec3-sub2:infer-step:-fitt}, but with the fitted
Mat\'ern cluster process as the null hypothesis model. It can be seen
that for all VOIs, the the empirically estimated PCFs fall inside the
envelopes at all interpoint distances. Based on this, we conclude that
the fits are fairly good.
...
\begin{figure}[!htb]
  ...
  \caption{Histograms of the half lengths $L_a$, $L_b$, $L_c$ and the
    ...
    reconstructed ellipsoids for the three bCT VOIs shown in
    Figure~\ref{fig:bct-datasets-recon}. Visual observations show that
    these parameters seem to follow either Gaussian or uniform
    distributions.}
