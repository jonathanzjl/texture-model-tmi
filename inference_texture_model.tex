...
\author{Zhijin~Li, Ann-Katherine~Carton, Serge~Muller,
  ...
  \thanks{Zhijin Li is with GE Healthcare France, Buc, 78530,
    France. E-mail: jonathan.li@ge.com.}% <-this % stops a space
  ...
  \thanks{Ann-Katherine~Carton, Serge~Muller, Thomas~Almecija and
    ...
    78530, France.}% <-this % stops a space
...
% This might be due to the subjective model parameters, which might
% not correctly capture the variability of breast tissue in clinical
% bCT volumes. To allow the 3D solid breast texture model to simulate
% a larger variability of breast textures, we propose to infer the
% model parameters from clinical bCT volumes.
...
% Notice that $L_{e}(K) = K$ if $K$ is itself an ellipsoid.

% The shift of $\mathcal{E}$ consists in replacing it by the Legendre
% ellipsoid $\mathcal{L}(K_{\mathcal{E}}(\mathcal{D}))$ of
% $K_{\mathcal{E}}(\mathcal{D})$, with


% To describe the shift step used in our adapted multiple birth, death
% and shift (MBDS) algorithm, we adopt the notion of \textit{Legendre
% ellipsoid} of a convex body in classical mechanics
% \cite{lutwak2000new}. Given a convex body $K \in \mathbb{R}^3$, the
% Legendre ellipsoid $L_{e}(K)$ is defined as
% \cite{ludwig2003ellipsoids}:
% \begin{equation}
%   \label{eq:ellip-approx}
%   L_{e}(K) = \{ x \in K | (x-\mu)^T \Sigma^{-1} (x-\mu) \leq
%   1 \},
% \end{equation}
% where
% $$
% \mu = \frac{\int_{x \in K} x dx}{\nu (K) }
% $$
% and
% $$
% \Sigma = \frac{\int_{x \in K} (x - \mu)(x - \mu)^T dx} {\nu(K)}.
% $$
% Notice that $L_{e}(K) = K$ if $K$ is itself an ellipsoid.

% Let $\mathcal{D}_{\mathcal{E}}$ be the binary volume representing
% voxelized ellipsoid $\mathcal{E}$. The shift of $\mathcal{E}$
% consists in replacing it by the Legendre ellipsoid
...
% set of reconstructed ellipsoid centers $\Phi_s$,
...
\begin{figure}[!htb]
  ...
  \raisebox{0.5em}{\fontsize{9}{9}\selectfont \rotatebox{90}{Ground
      truth}} \includegraphics[width=0.14\textwidth]
...
\begin{figure}[!htb]
  ...
  \hspace{1mm} \parbox{2.8cm}{\fontsize{9}{9}\selectfont \centering
    ...
    \centering Mammographic projections}
  \parbox{2.75cm}{\fontsize{9}{9}\selectfont \centering DBT
    reconstructed slices} \vspace{1mm}
